...
class Queue extends EventEmitter {
  ...
  async enqueueAt (timestamp, q, func, args) {
    ...
    let delayedKey = `delayed:${q}:${rTimestamp}`
    let timestampsKey = `timestamps:${q}:${item}`

    let match = (delayedKey)
    let foundMatch = await this.connection.redis.sismember(this.connection.key(timestampsKey), match)
    if (foundMatch === 1) { throw new Error('Job already enqueued at this time with same arguments') }

    await this.connection.redis.rpush(this.connection.key(delayedKey), item)
    await this.connection.redis.sadd(this.connection.key(timestampsKey), (delayedKey))
    await this.connection.redis.zadd(this.connection.key(`delayed_queue_schedule:${q}`), rTimestamp, rTimestamp)
  }
  ...
  async delayedAt (timestamp, q) {
    let rTimestamp = Math.round(timestamp / 1000) // assume timestamp is in ms
    let items = await this.connection.redis.lrange(this.connection.key(`delayed:${q}:${rTimestamp}`), 0, -1)
  ...
  async allDelayed (q) {
    ...
    for (let i in timestamps) {
      ...
      let {tasks, rTimestamp} = await this.delayedAt(timestamp, q)
